...
const PopularPostCard = ({ post, rank }: PopularPostCardProps) => {
  ...
  const hoverBorderStyles = {
    1: "hover:border-yellow-500/50",
    2: "hover:border-slate-400/50",
    3: "hover:border-orange-700/50",
  };
  
  const hoverTextStyles = {
      1: "group-hover:text-yellow-500",
      2: "group-hover:text-slate-400",
      3: "group-hover:text-orange-600",
  };
  ...
  // @ts-ignore
  const hoverBorderClass = hoverBorderStyles[rank] || "hover:border-primary/50";
  // @ts-ignore
  const hoverTextClass = hoverTextStyles[rank] || "group-hover:text-primary";
  // @ts-ignore
  const borderColor = rankColors[rank] || 'hsl(var(--primary))';
