...
import { getDiaryEntryClient, DiaryEntry } from '@/lib/data';
...
export default function EditDiaryEntryPage() {
  ...
  useEffect(() => {
    if (id) {
        getDiaryEntryClient(id).then(data => {
            if(data) {
                setEntry(data);
                form.reset(data);
                setIconPreview(data.icon);
            } else {
                toast({title: "Error", description: "Diary entry not found.", variant: "destructive"});
                router.push('/admin');
            }
        });
    }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
        ...
        toast({
            ...
            description: `Chapter ${entry?.chapter} has been successfully updated.`,
        });
        router.push(`/admin`);
    } catch (error) {
       toast({
    ...
    <div className="container mx-auto px-4 py-16">
        ...
        <Card className="glass-card max-w-3xl mx-auto">
            <CardHeader>
                ...
                <CardDescription>Make changes to this diary chapter.</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
