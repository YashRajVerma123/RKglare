...
import { getBulletin, getBulletinClient } from '@/lib/data';
...
export default function EditBulletinPage() {
  ...
  useEffect(() => {
    if (id) {
        getBulletinClient(id).then(data => {
            if (data) {
                setBulletin(data);
                form.reset({
                    title: data.title,
                ...
                toast({ title: "Error", description: "Bulletin not found.", variant: "destructive"});
                router.push('/admin');
            }
        });
    }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    try {
        ...
        toast({
            ...
            description: `The bulletin has been successfully updated.`,
        });
        router.push(`/admin`);
    } catch (error) {
       toast({
    ...
    <div className="container mx-auto px-4 py-16">
        ...
        <Card className="glass-card max-w-3xl mx-auto">
            <CardHeader>
                ...
                <CardDescription>Make changes to the bulletin below.</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
